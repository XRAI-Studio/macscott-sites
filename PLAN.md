...
- **Middleware rewrites by host to tenant-prefixed internal routes:** `scott.macscott.net/blog` → `/_t/scott/blog`, `alexander.macscott.net/apps` → `/_t/alexander/apps`. App routes live under `app/_t/[tenant]/...`. This makes the tenant part of the URL cache key, so ISR/static generation/RSC payloads can never leak across domains. Direct navigation to `/_t/...` on a public host redirects to the clean URL.
...
- An app opts in with GitHub topic **`macscott-app`** + a **`macscott.json`** manifest at repo root:
  ...
  ```
- **Validation is strict (zod):** `schemaVersion` required; length caps on strings; `liveUrl` HTTPS-only; `accent` a valid hex color; `screenshot` a normalized in-repo relative path; unknown fields ignored; invalid manifest → repo excluded with a logged, per-repo rejection reason.
...
- Caching: catalog cached via Next data cache, ISR revalidate ~1h, keyed per tenant via the `/_t/[tenant]` route structure.
- **Instant refresh:** `POST /api/revalidate` authenticated by a bearer secret **in a header** (never a query string). A tiny reusable GitHub Actions workflow (documented in README, ~6 lines) is added to app repos to call it on push — push a game → live in seconds; ISR is the no-Action fallback.
...
- **Accessibility + SEO:** the nebula is a progressive enhancement over a real, always-present semantic catalog — a keyboard-navigable, screen-reader-visible DOM list of apps (styled as the fallback grid when WebGL is off; visually hidden but focusable when the canvas runs). Crawlers see every app.
...
Unit tests (vitest) for the risk core: manifest zod validation (incl. owner anti-spoof, HTTPS-only, bad hex), tenant resolution from host (allowlist, unknown host, prod `?tenant=` rejection), blog frontmatter validation + slug dedupe + markdown sanitization, catalog builder partial-failure behavior (mocked GitHub). E2E left manual for v1.
