...
import { Float, Html, Image as DreiImage, MeshTransmissionMaterial, OrbitControls, Stars } from "@react-three/drei";
...
function PreviewPlane({ url }: { url: string }) {
  return <DreiImage url={url} scale={[1.25, .78]} position={[0, 0, .14]} transparent />;
}

function Orb({ app, index, tier, reducedMotion }: { app: CatalogApp; index: number; tier: Tier; reducedMotion: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    if (diving.current) {
      ...
      if (diveElapsed.current > .7) { diving.current = false; sessionStorage.setItem("macscott:last-app", app.slug); router.push(`/apps/${app.slug}`); }
      return;
    }
  ...
  return (
    <Float speed={reducedMotion ? 0 : .8} rotationIntensity={reducedMotion ? 0 : .18} floatIntensity={reducedMotion ? 0 : .35}>
      <group ref={group} position={position} onClick={(event) => { event.stopPropagation(); sessionStorage.setItem("macscott:last-app", app.slug); if (reducedMotion) router.push(`/apps/${app.slug}`); else diving.current = true; }} onPointerEnter={() => setHovered(true)} onPointerLeave={() => setHovered(false)}>
        <mesh scale={hovered ? 1.12 : 1}>
          <sphereGeometry args={[.86, tier === "full" ? 48 : 24, tier === "full" ? 48 : 24]} />
          {tier === "full" ? <MeshTransmissionMaterial backside chromaticAberration={.12} distortion={.25} distortionScale={.25} thickness={.5} roughness={.08} color={app.accent} transmission={1} /> : <meshStandardMaterial color={app.accent} metalness={.3} roughness={.22} transparent opacity={.72} />}
        </mesh>
...
export default function OrbNebula({ apps, tier, reducedMotion, onContextLost }: { apps: CatalogApp[]; tier: Tier; reducedMotion: boolean; onContextLost: () => void }) {
  ...
  return (
    <Canvas dpr={tier === "full" ? [1, 1.75] : 1} camera={{ position: [0, 0, 8], fov: 55 }} gl={{ antialias: tier === "full", powerPreference: tier === "full" ? "high-performance" : "low-power" }} onCreated={({ gl }) => gl.domElement.addEventListener("webglcontextlost", (event) => { event.preventDefault(); onContextLost(); }, { once: true })}>
      ...
      {apps.map((app, index) => <Orb app={app} index={index} key={app.slug} reducedMotion={reducedMotion} tier={tier} />)}
      <OrbitControls enablePan={false} enableDamping dampingFactor={.06} minDistance={3} maxDistance={14} autoRotate={!reducedMotion} autoRotateSpeed={.18} />
