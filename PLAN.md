...
- **Middleware matcher excludes non-page requests:** `_next/*`, `/api/*`, static assets, favicon/icons — only page navigations are rewritten. **`robots.txt`, `sitemap.xml`, and RSS ARE tenant-rewritten** (they emit host-specific URLs and must resolve per domain).
- Host resolution uses the Vercel-forwarded host, matched against an **allowlist** (the two production domains + `*.vercel.app` previews + localhost). Unknown hosts → redirect to `scott.macscott.net` (no content served under unrecognized hosts). Preview/localhost default to `scott` with a `?tenant=` override that is **disabled in production** (`VERCEL_ENV === 'production'`). **`*.vercel.app` on a PRODUCTION deployment 301-redirects to the canonical custom domain; actual preview deployments render with `noindex` + production canonicals** — no duplicate-content leak from either alias class.
...
- **Stable app identity:** slug = `<account>--<repo>` (GitHub account + repo name verbatim, lowercased — both are already URL-safe and unique as a pair; no kebab collapsing, no collision-dependent renames, never derived from the mutable `title`). Deep links are stable for the life of the repo.
...
- **Catalog builder** (server-side): list each account's repos via REST (`per_page=100`, paginated; exclude forks and archived repos; cap 100 apps), filter by topic, then fetch `macscott.json` (and screenshot URL) **pinned to the repo's current default-branch commit SHA** so manifest+screenshot are never mixed across commits. **Failure semantics:** deterministic rejections (invalid/missing manifest) exclude only that repo, with a logged reason; **transient failures (rate limit, timeout, 5xx, account-listing failure) abort the whole refresh and keep serving the last good catalog** — apps never silently vanish because GitHub hiccuped. Bounded concurrency per-repo; stable sort (owner, then title).
...
- **Embedding policy:** iframe with `sandbox="allow-scripts allow-same-origin allow-pointer-lock allow-popups"` (no top-navigation), a restrictive Permissions-Policy `allow` list, and `referrerpolicy="no-referrer"`. **Showcase pages send `Content-Security-Policy: frame-ancestors 'none'`** — even if an embedded app navigates its frame to a showcase URL, the showcase refuses to render inside any iframe, closing the same-origin sandbox-escape path. Manifest `embeddable: false` (or absent `liveUrl`) → no iframe. **An "open in new tab ↗" control is always visible** on every embedded app — no fragile runtime frame-block detection; if an app turns out non-embeddable, flip `embeddable: false` in its manifest. Origins are inherently allowlisted: only manifests from the brothers' own repos, HTTPS-only, pass validation.
...
- **Hostname tenanting via middleware rewrite to `/t/[tenant]/...`** — tenant is in the cache key by construction; no cross-domain cache leakage. Cost: one rewrite layer.
